...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
    ...
    while True:
        for s in range(nS):
            a = policy[s]
            value_function[s] = P[s][a][0][2] +  (gamma * P[s][a][0][0] * prev_value_function[P[s][a][0][1]])
        max_diff = 0
        for s in range(nS):
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
    ...
    for s in range(nS):
        max = 0
        for a in range(nA):
            action_value[s][a] = P[s][a][0][2] + (gamma * P[s][a][0][0] * value_from_policy[P[s][a][0][1]])
...
def policy_iteration(P, nS, nA, gamma=0.9, tol=10e-3):
    ...
    while(flag == 1):
        ...
        if ((i > 25) and (np.array_equal(np.array(value_function), np.array(prev_value_function)) == True)):
            flag = 0
        prev_value_function = value_function.copy()
        i += 1
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
    ...
    while True:
        for s in range(nS):
            ...
            for a in range(nA):
                value_function[s] = P[s][a][0][2] + (gamma * P[s][a][0][0] * prev_value_function[P[s][a][0][1]])
                if(value_function[s] > max):
                    max = value_function[s]
                    policy[s] = a
